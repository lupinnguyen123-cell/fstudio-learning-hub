...
export default async (request: Request) => {
  ...
  try {
    ...
    try { const config = readProviderConfig(); const response = await createProvider(config).generateCourseDraft(input); logResult({ requestId, provider: response.provider, model: response.model, status: 'success', latencyMs: response.latencyMs, sourceLength: input.sourceDocument.extractedText.length, valid: true }); return json(response) }
    catch (error) {
      if (process.env.AI_ENABLE_MOCK_FALLBACK !== 'true' || process.env.CONTEXT === 'production') throw error
      const started = Date.now(); const service = new AiCourseService(null); const source = { ...input.sourceDocument, fileSize: input.sourceDocument.extractedText.length, importedAt: new Date().toISOString(), sourceLanguage: input.language, metadata: { extractionMode: input.sourceDocument.extractionMode, note: 'Server development mock fallback.' } } as const; const generatedAt = new Date().toISOString(); const draft = service.generateCourseDraft(source, input.courseType); const response = { requestId: input.requestId, provider: 'mock', model: 'deterministic-local', draft: { ...draft, generation: { provider: 'mock', model: 'deterministic-local', generatedAt, latencyMs: Date.now() - started, isMock: true } }, warnings: ['Đây là kết quả mô phỏng, không phải AI provider thật.'], latencyMs: Date.now() - started, generatedAt, isMock: true }; logResult({ requestId, provider: 'mock', model: 'deterministic-local', status: 'mock_fallback', latencyMs: response.latencyMs, sourceLength: input.sourceDocument.extractedText.length, valid: true }); return json(response)
    }

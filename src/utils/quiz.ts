...
export const isPassingScore = (score: number): boolean => score >= PASS_SCORE
...
export function gradeQuiz(courseId: string, questions: Question[], answers: Record<string, QuizAnswerValue>, attemptNumber: number, now = new Date()): QuizResult {
  ...
  return { courseId, score, correctAnswers, incorrectAnswers: questions.length - correctAnswers - unanswered, unanswered, totalQuestions: questions.length, passed: isPassingScore(score), completedAt: now.toISOString(), attemptNumber, answers: reviews }
}

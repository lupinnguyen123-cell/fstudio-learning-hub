...
export class DocumentService {
  ...
  toAiHandoff(document: StructuredDocument): DocumentHandoff {
    return { schemaVersion: 1, sourceId: document.source.id, fileName: document.source.fileName, format: document.source.format, metadata: { title: document.metadata.title, pageCount: document.metadata.pageCount, slideCount: document.metadata.slideCount, wordCount: document.metadata.wordCount }, sections: document.pages.flatMap((page) => page.sections.map((section) => ({ title: section.title, level: section.level, elements: section.elements.filter((element) => element.type !== 'blank_line').map((element) => ({ type: element.type, content: elementContent(element), retailSignals: element.retailSignals })) }))), normalizedText: document.normalizedText, detectedSignals: document.detectedSignals, qualityScore: document.quality.score }
  }

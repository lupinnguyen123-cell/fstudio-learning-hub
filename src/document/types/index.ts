...
export interface DocumentHandoff { schemaVersion: 1; sourceId: string; fileName: string; format: DocumentFormat; metadata: Pick<DocumentMetadata, 'title' | 'pageCount' | 'slideCount' | 'wordCount'>; sections: Array<{ title: string; level: number; elements: Array<{ type: DocumentElement['type']; content: string; retailSignals: RetailSignal[] }> }>; normalizedText: string; detectedSignals: RetailSignal[]; qualityScore: number }

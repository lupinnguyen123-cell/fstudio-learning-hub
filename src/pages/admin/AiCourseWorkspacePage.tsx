...
export function AiCourseWorkspacePage() {
  ...
  const importFile = async (selected?: File) => {
    ...
    try {
      ...
      setImported(analysis); setSourceText(extractedText); setPreset(analysis.suggestedPreset); setDraft(null); setPhase('import')
      setNotice(extractedText ? 'Đã đọc text từ file. Hãy kiểm tra trước khi Generate.' : 'Định dạng này chưa có parser thật. Dán extracted text để dùng AI provider.')
    } catch { setNotice('Định dạng chưa được hỗ trợ. Chọn PPTX, PDF, DOCX, TXT hoặc Markdown.') }
  }
  const preparePastedText = () => {
    try { const analysis = aiCourseService.analyzeText(sourceText); setImported(analysis); setPreset(analysis.suggestedPreset); setNotice('Nội dung dán đã sẵn sàng.') }
    catch { setNotice('Hãy nhập nội dung nguồn trước khi tiếp tục.') }
  }

  const generateRequest = (analysis: AiImportedFile): GenerateCourseRequest => ({ sourceDocument: { id: analysis.source.id, fileName: analysis.source.fileName, fileType: analysis.source.fileType, extractedText: analysis.source.extractedText, extractionMode: analysis.source.metadata.extractionMode }, courseType: preset, language: 'vi', tone: 'professional_friendly', audience: 'Nhân viên tư vấn F.Studio', desiredModuleCount: analysis.estimatedModules, desiredLessonLength: 'short', includeQuiz: true, includeScenario: true, includeFlashcards: true, retailContext: 'Đào tạo bán lẻ F.Studio, ưu tiên áp dụng tại quầy.', requestId: requestId() })
  const process = async () => {
    let analysis = imported
    if (sourceText.trim() && (!analysis || analysis.source.extractedText !== sourceText.trim())) { try { analysis = aiCourseService.analyzeText(sourceText); setImported(analysis) } catch { analysis = null } }
    if (!analysis) { setNotice('Hãy dán nội dung nguồn hoặc chọn file TXT/Markdown trước.'); return }
    if (analysis.source.metadata.extractionMode === 'mock') { setNotice('PDF/PPTX/DOCX chưa được parse thật. Hãy dán extracted text; hệ thống sẽ không giả vờ đã đọc file.'); return }
    ...
    try {
      const response = await aiClient.generateCourse(generateRequest(analysis), controller.signal)

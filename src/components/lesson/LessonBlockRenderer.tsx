...
function ChoiceActivity({ block }: { block: Extract<LessonBlock, { type: 'quick_question' | 'multiple_choice' | 'multi_select' }> }) {
  ...
  return <section className="interactive-block interactive-block--quiz"><span className="block-kicker"><HelpCircle />{block.type === 'quick_question' ? 'Câu hỏi nhanh' : 'Tương tác'}</span><h2>{block.question}</h2><div className="activity-options">{block.options.map((option) => <label key={option.id} className={selected.includes(option.id) ? 'selected' : ''}><input type={multiple ? 'checkbox' : 'radio'} name={block.id} checked={selected.includes(option.id)} onChange={() => toggle(option.id)} />{option.text}</label>)}</div><button className="button button-primary" type="button" disabled={!selected.length} onClick={() => setChecked(true)}>Kiểm tra</button>{checked && <div className={`activity-feedback ${correct ? 'correct' : 'incorrect'}`} role="status"><strong>{correct ? 'Chính xác!' : 'Chưa chính xác'}</strong><p>{correct ? block.options.filter((item) => item.correct).map((item) => item.feedback).join(' ') : block.options.find((item) => selected.includes(item.id))?.feedback}</p><p>{block.explanation}</p></div>}</section>
}
function Flashcards({ block }: { block: Extract<LessonBlock, { type: 'flashcard' }> }) { const [index, setIndex] = useState(0); const [flipped, setFlipped] = useState(false); const card = block.cards[index]; if (!card) return null; return <section className="interactive-block interactive-block--flashcard"><span className="block-kicker">Flashcard · {index + 1}/{block.cards.length}</span><h2>{block.title}</h2><button type="button" className={`flashcard ${flipped ? 'flipped' : ''}`} onClick={() => setFlipped(!flipped)}>{card.imageUrl && <img src={card.imageUrl} alt="" />}<strong>{flipped ? card.back : card.front}</strong><small>Chạm để lật</small></button><div className="inline-actions"><button type="button" className="button button-secondary" disabled={index === 0} onClick={() => { setIndex(index - 1); setFlipped(false) }}>Trước</button><button type="button" className="button button-secondary" disabled={index === block.cards.length - 1} onClick={() => { setIndex(index + 1); setFlipped(false) }}>Sau</button></div></section> }
...
export function LessonBlockRenderer({ block }: { block: LessonBlock }) {
  ...
  switch (block.type) {
    case 'heading': return <SectionHeader title={block.text} level={block.level} eyebrow={block.level === 2 ? 'Kiến thức chính' : 'Ghi nhớ'} />
    ...
    case 'video': { const embed = safeEmbed(block.url, block.provider); return <figure className="video-block block-accent-video"><span className="block-kicker"><PlaySquare />Video</span><h2>{block.title}</h2>{embed ? block.provider === 'direct' ? <video controls preload="metadata" src={embed}>Trình duyệt không hỗ trợ video.</video> : <iframe src={embed} title={block.title} allow="accelerometer; autoplay; encrypted-media; picture-in-picture" allowFullScreen /> : <div className="media-placeholder"><PlaySquare /><span>Video URL chưa hợp lệ hoặc không tải được.</span></div>}<figcaption>{block.description} · {block.durationMinutes} phút{block.required ? ' · Bắt buộc xem' : ''}</figcaption>{block.transcript && <details><summary>Transcript</summary><p>{block.transcript}</p></details>}</figure> }
    case 'bullet_list': return <section className="content-panel block-accent-knowledge"><span className="block-kicker"><Lightbulb />Kiến thức</span><h2>{block.title}</h2><ul>{block.items.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ul></section>
    ...
    case 'checklist': return <section className="interactive-block interactive-block--checklist"><span className="block-kicker"><Check />Checklist</span><h2>{block.title}</h2>{block.items.map((item) => <label className="check-row" key={item.id}><input type="checkbox" />{item.text}</label>)}</section>
